...
function Map:draw(tx, ty, sx, sy)
    ...
    lg.clear(0, 0, 0, 0)  -- Clear with full transparency
    ...
    for _, layer in ipairs(self.layers) do
        if layer.visible and layer.opacity > 0 then
            local px = layer.parallaxx or 1
            local py = layer.parallaxy or 1
            px, py = math.floor(tx * px), math.floor(ty * py)
            
            lg.push()
            lg.translate(px, py)
            
            -- Clear any existing transformations for the layer
            layer.sx = nil
            layer.sy = nil
...
function Map:drawTileLayer(layer)
	...
	for _, batch in pairs(layer.batches) do
        -- Add support for layer scale
        local sx = layer.sx or 1
        local sy = layer.sy or 1
        -- When drawing with negative scale, we need to adjust the position
        local x = floor(layer.x)
        local y = floor(layer.y)
        if sx < 0 then
            x = x + layer.width * self.tilewidth
        end
        if sy < 0 then
            y = y + layer.height * self.tileheight
        end
        lg.draw(batch, x, y, 0, sx, sy)
    end
end
